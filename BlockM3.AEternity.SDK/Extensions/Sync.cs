...
namespace BlockM3.AEternity.SDK.Extensions
{
    public static class Sync
    {
        ...
        public static InProgress<OracleQuery<T, S>> RegisterOracle<T, S>(this ClientModels.Account account, ulong queryFee = Constants.BaseConstants.ORACLE_QUERY_FEE, ulong fee = Constants.BaseConstants.FEE, Ttl ttl = default(Ttl), ushort abiVersion = Constants.BaseConstants.ORACLE_VM_VERSION) => account.RegisterOracleAsync<T, S>(queryFee, fee, ttl, abiVersion).RunAndUnwrap();
        public static OracleQuery<T, S> GetOwnOracle<T, S>(this ClientModels.Account account) => account.GetOwnOracleAsync<T, S>().RunAndUnwrap();
        public static RegisteredOracle<T, S> GetOracle<T, S>(this ClientModels.Account account, string oraclepubkey) => account.GetOracleAsync<T, S>(oraclepubkey).RunAndUnwrap();
        ...
        public static List<OracleQuestion<T, S>> Query<T, S>(this OracleQuery<T, S> oracleQuery, ushort? limit, string lastQueryId = null) => oracleQuery.QueryAsync(limit, lastQueryId).RunAndUnwrap();
        public static InProgress<bool> Extend<T, S>(this OracleQuery<T, S> oracleQuery, ulong fee = Constants.BaseConstants.FEE, ulong extendTtl = Constants.BaseConstants.ORACLE_RESPONSE_TTL_VALUE) => oracleQuery.ExtendAsync(fee, extendTtl).RunAndUnwrap();
        ...
        public static InProgress<S> Ask<T, S>(this RegisteredOracle<T, S> oracle, T query, ulong fee = Constants.BaseConstants.ORACLE_QUERY_FEE, TTLType queryTtlType = TTLType.Delta, ulong queryTtl = Constants.BaseConstants.ORACLE_QUERY_TTL_VALUE, ulong responseRelativeTtl = Constants.BaseConstants.ORACLE_RESPONSE_TTL_VALUE) => oracle.AskAsync(query, fee, queryTtlType, queryTtl, responseRelativeTtl).RunAndUnwrap();
