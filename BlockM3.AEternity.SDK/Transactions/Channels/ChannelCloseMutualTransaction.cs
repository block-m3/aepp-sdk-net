...
{
    public class ChannelCloseMutualTransaction : Transaction<UnsignedTx, ChannelCloseMutualTx>
    {
        internal ChannelCloseMutualTransaction(ILoggerFactory factory, FlatClient client) : base(factory, client)
        {
        }
        ...
        {
            return _client.CreateDebugChannelCloseMutualAsync(Model, token);
        }

        public override byte[] Serialize()
        {
            RLPEncoder enc = new RLPEncoder();
            enc.AddInt(Constants.SerializationTags.OBJECT_TAG_CHANNEL_CLOSE_MUTUAL_TRANSACTION);
